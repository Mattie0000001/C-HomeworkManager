/****************************************************************************
** Meta object code from reading C++ file 'stu.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.9.9)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../stu.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'stu.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.9.9. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_Stu_t {
    QByteArrayData data[11];
    char stringdata0[178];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_Stu_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_Stu_t qt_meta_stringdata_Stu = {
    {
QT_MOC_LITERAL(0, 0, 3), // "Stu"
QT_MOC_LITERAL(1, 4, 15), // "mainwindow_show"
QT_MOC_LITERAL(2, 20, 0), // ""
QT_MOC_LITERAL(3, 21, 31), // "on_infor_backpushButton_clicked"
QT_MOC_LITERAL(4, 53, 17), // "on_update_clicked"
QT_MOC_LITERAL(5, 71, 15), // "on_back_clicked"
QT_MOC_LITERAL(6, 87, 16), // "on_cal_2_clicked"
QT_MOC_LITERAL(7, 104, 15), // "on_PS_3_clicked"
QT_MOC_LITERAL(8, 120, 15), // "on_ED_3_clicked"
QT_MOC_LITERAL(9, 136, 20), // "on_circuit_3_clicked"
QT_MOC_LITERAL(10, 157, 20) // "on_physics_3_clicked"

    },
    "Stu\0mainwindow_show\0\0"
    "on_infor_backpushButton_clicked\0"
    "on_update_clicked\0on_back_clicked\0"
    "on_cal_2_clicked\0on_PS_3_clicked\0"
    "on_ED_3_clicked\0on_circuit_3_clicked\0"
    "on_physics_3_clicked"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Stu[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       9,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   59,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       3,    0,   60,    2, 0x08 /* Private */,
       4,    0,   61,    2, 0x08 /* Private */,
       5,    0,   62,    2, 0x08 /* Private */,
       6,    0,   63,    2, 0x08 /* Private */,
       7,    0,   64,    2, 0x08 /* Private */,
       8,    0,   65,    2, 0x08 /* Private */,
       9,    0,   66,    2, 0x08 /* Private */,
      10,    0,   67,    2, 0x08 /* Private */,

 // signals: parameters
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void Stu::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Stu *_t = static_cast<Stu *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->mainwindow_show(); break;
        case 1: _t->on_infor_backpushButton_clicked(); break;
        case 2: _t->on_update_clicked(); break;
        case 3: _t->on_back_clicked(); break;
        case 4: _t->on_cal_2_clicked(); break;
        case 5: _t->on_PS_3_clicked(); break;
        case 6: _t->on_ED_3_clicked(); break;
        case 7: _t->on_circuit_3_clicked(); break;
        case 8: _t->on_physics_3_clicked(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            typedef void (Stu::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&Stu::mainwindow_show)) {
                *result = 0;
                return;
            }
        }
    }
    Q_UNUSED(_a);
}

const QMetaObject Stu::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_Stu.data,
      qt_meta_data_Stu,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *Stu::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Stu::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_Stu.stringdata0))
        return static_cast<void*>(this);
    return QWidget::qt_metacast(_clname);
}

int Stu::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 9)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 9;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 9)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 9;
    }
    return _id;
}

// SIGNAL 0
void Stu::mainwindow_show()
{
    QMetaObject::activate(this, &staticMetaObject, 0, nullptr);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
